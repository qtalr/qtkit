...
test_that("valid inputs", {
  expect_silent(validate_inputs_gtd(valid_name, valid_path, valid_dir, FALSE, TRUE))
})

# invalid corpus name
test_that("invalid corpus name", {
  expect_error(validate_inputs_gtd(invalid_name, valid_path, valid_dir, FALSE, TRUE))
})

# invalid corpus path
test_that("invalid corpus path", {
  expect_error(validate_inputs_gtd(valid_name, invalid_path, valid_dir, FALSE, TRUE))
})

# invalid boolean force
test_that("invalid boolean force", {
  expect_error(validate_inputs_gtd(valid_name, valid_path, valid_dir, NULL, TRUE))
})

# invalid boolean confirmed
test_that("invalid boolean confirmed", {
  expect_error(validate_inputs_gtd(valid_name, valid_path, valid_dir, FALSE, NULL))
})
...
test_that("write_to_disk successfully writes data to disk", {
  ...
  result <- write_to_disk(mock_func_success, corpus_name, corpus_path, file_path, timeout = 5)
  expect_null(result)
...
test_that("write_to_disk handles timeout correctly", {
  ...
  result <- write_to_disk(getTokens, corpus_name, corpus_path, file_path, timeout = 1)
  expect_null(result)
  expect_message(write_to_disk(getTokens, corpus_name, corpus_path, file_path, timeout = 1), regex = "timed out at 1 seconds")
})
...
test_that("write_to_disk does not write to disk on function error", {
  ...
  corpus_path <- c("ca", "CABNC")
  file_path <- file.path(temp_dir, "test.csv")
  write_to_disk(mock_func_error, corpus_name, corpus_path, file_path, timeout = 5)
...
test_that("write_to_disk handles empty input data correctly", {
  ...
  result <- write_to_disk(mock_func_empty, corpus_name, corpus_path, file_path, timeout = 5)
