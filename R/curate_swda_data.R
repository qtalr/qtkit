...
curate_swda_file <- function(file) {
  ...
  speaker_info_chr <-
    unlist(
      strsplit(
        sub(".*\\t", "", grep("\\d+_\\d+_\\d+", doc_chr, value = TRUE)),
        "_"
      )
    )
  ...
  text <-
    grep(".+", trimws(doc_chr[text_start_index:text_end_index]), value = TRUE)

  swda_df <-
    data.frame(doc_id = doc_id, text = text, stringsAsFactors = FALSE)
  ...
  swda_df$speaker_id <-
    ifelse(swda_df$speaker == "A", speaker_a_id, speaker_b_id)

  # Select relevant columns
  swda_df <- swda_df[, c(
    "doc_id", "damsl_tag", "speaker_id", "speaker", "turn_num",
