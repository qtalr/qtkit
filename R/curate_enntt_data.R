...
#'
#' @param dir_path A string. The path to the directory containing the data files.
#'
...
#'
#' @export
# TODO: curate_enntt_data function
curate_enntt_data <- function(dir_path) {
    files <- basename(list.files(path = dir_path, pattern = "(dat|tok)$", full.names = TRUE,
        recursive = TRUE))
...
curate_enntt_file <- function(dir_path, type) {
    # Create file paths
    dat_file <- paste0(dir_path, "/", type, ".dat")
    tok_file <- paste0(dir_path, "/", type, ".tok")
...
extract_dat_attrs <- function(line_node) {
    ...
    data.frame(session_id, speaker_id, state, session_seq, stringsAsFactors = FALSE)
}
